...
export default function SummaryPanel({
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      try {
        logger.debug("[Activity] Fetching team activity");
        const res = await fetch(`${API_BASE_URL}/api/team/activity`, {
          credentials: "include",
        });
        if (!res.ok) {
          logger.error("[Activity] Fetch failed:", res.status);
          return;
        }
        ...
        const events = members
          ...
          .map((m) => {
            const at = m.last_activity.at
              ...
              : null;
            // Backend now returns AI summaries, so we don't need to truncate
            const detail = m.last_activity.message?.trim() || "[No recent activity]";
        ...
        if (!cancelled) {
          ...
          logger.debug(
            "[Activity] ✅ Updated with",
            events.length,
            "activities"
          );
        }
      } catch (err) {
        logger.error("[Activity] Error fetching:", err);
      }
    };
  ...
  return (
    <aside className="summary-panel glass">
      ...
      {/* Activity history is rendered below in the sidebar layout */}
    </aside>
  );
}
